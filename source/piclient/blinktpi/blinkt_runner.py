...
def init():
    global handler
    if settings.is_fake():
        from blinktpi import blinktshandler_faker
        handler = blinkthandler_faker
    else:
        from blinktpi import blinkthandler
        handler = blinkthandler
    return
...
def set_status(connected, maledoor, femaledoor):
    if connected == 'false':
        handler.display_unknown()
    elif maledoor == 'open' and femaledoor == 'open':
        handler.display_both_open()
    elif maledoor == 'closed' and femaledoor == 'open':
        handler.display_male_closed()
    elif maledoor == 'open' and femaledoor == 'closed':
        handler.display_female_closed()
    else:
        handler.display_both_closed()
