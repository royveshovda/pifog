...
window.shadowConnectHandler = function() {
   ...
   setTimeout(function() {
    ...
    // TODO: Test if MQTT-subscribe directly to '$aws/things/DoorPi/shadow/update/accepted' will work
};
...
function set_sensor_state(ppm, temp, hum, loud, timestamp) {
  ...
  if( !isNaN(newGaugeVal) ) {
    ...
    if( !$('.tempGauge').is(':visible') ){
      ...
      $('.tempGauge').show();
      var newTempVal = parseFloat(temp);
      if( !isNaN(newTempVal) ) {
        $('.tempGauge').tempgauge('updatetemp', newTempVal);
      }
    }
  }
